...
class PromptOptimizer {
    ...
    async optimizeForContext(text, context = 'llm-interaction', goals = ['clarity'], options = {}) {
        ...
        // Keyed tiers: backend-validated. Free (no-key) tier: local-only, gated by
        // a local daily counter (see licenseManager.checkDailyQuota).
        let licenseResult;
        try {
            licenseResult = await this.licenseManager.validateLicense();

            if (!licenseResult.valid) {
                throw new Error(`License validation failed: ${licenseResult.error}`);
            }

            const quota = await this.licenseManager.checkDailyQuota(licenseResult);
            if (!quota.allowed) {
                throw new Error(`Daily optimization limit (${quota.limit}) reached. Upgrade to Pro for unlimited optimizations at https://promptoptimizer.xyz/local-license`);
            }

            const remainingText = quota.remaining === Infinity ? 'Unlimited' : `${quota.remaining}/${quota.limit}`;
            this.logger.info(`✅ License validated (${licenseResult.tier || licenseResult.type}). Remaining optimizations: ${remainingText}`);
        ...
        try {
            ...
            // Only the free tier is locally metered — keyed tiers are tracked by the backend.
            if (licenseResult.type === 'free') {
                await this.licenseManager.incrementDailyUsage();
            }
