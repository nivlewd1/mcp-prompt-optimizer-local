...
class MCPToolHandler {
    ...
    // The SDK's own Protocol layer already wraps whatever setRequestHandler's
    // callback returns in a {jsonrpc, result, id} envelope (see
    // node_modules/@modelcontextprotocol/sdk/dist/cjs/shared/protocol.js,
    // Protocol._onrequest). Building a second envelope here double-wraps
    // every tool response, burying the real {content:[...]} payload one
    // level too deep for any spec-compliant client to find. Return the
    // bare CallToolResult shape instead and let the SDK do the framing.
    _createResponse(result, messageId) {
        return result;
    }

    // MCP reports tool-execution failures as a normal CallToolResult with
    // isError: true (so the calling LLM can see and react to the message),
    // not as a JSON-RPC protocol-level error — those are reserved for
    // transport/method-level failures the SDK itself raises.
